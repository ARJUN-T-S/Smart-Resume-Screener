...
const ResumeData = () => {
  ...
  useEffect(() => {
    const fetchComparisons = async () => {
      ...
      try {
        ...
        const comparisonsResponse = await fetch(`http://localhost:5000/other/getGroupsForJd/${resume.groupId}`, {
          headers: {
  ...
  const handleJobClick = async (jobComparison) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/job-desc/${jobComparison.jobId}`, {
        headers: {
          'Authorization': `Bearer ${idToken}`,
        },
      });
