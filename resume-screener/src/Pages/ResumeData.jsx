...
const ResumeData = () => {
  ...
  useEffect(() => {
    const fetchComparisons = async () => {
      ...
      try {
        ...
        const comparisonsResponse = await fetch(import.meta.env.VITE_VAPI_URL+`/other/getGroupsForJd/${resume.groupId}`, {
          headers: {
  ...
  const handleJobClick = async (jobComparison) => {
    try {
      ...
      const response = await fetch(import.meta.env.VITE_VAPI_URL+`/job-desc/${jobComparison.jobId}`, {
        headers: {
          'Authorization': `Bearer ${idToken}`,
        },
      });
